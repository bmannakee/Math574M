...
lars.obj <- cv.glmnet(X.train,Y.train,alpha=1,nfolds=15,family='binomial')
...
#lars.pred
coef(lars.obj,s='lambda.min')
count(as.character(lars.pred))
count(subset(df,test)$class)
mean(as.numeric(lars.pred)!=subset(df,test)$class)
...
mean(lda.pred$class!=subset(df,test)$enzyme)
...
mean(qda.pred$class!=subset(df,test)$enzyme)
               
# Try SVM
ksvm.obj <- ksvm(form2,data=subset(df,train),cross=30,C=5)
error(ksvm.obj)
testdf <- subset(df,test)
ksvm.pred <- predict(ksvm.obj,testdf,type='response')
mean(ksvm.pred!=testdf$enzyme)
summary(ksvm.pred)
